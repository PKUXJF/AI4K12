// src/services/kimiApi.ts - 使用成熟的 SiliconFlow 调用方案
...
export async function testKIMIConnection(
  ...
  const base = apiBase || localStorage.getItem('kimi_api_base') || 'https://api.siliconflow.cn/v1'
...
// 生成题目（带重试机制）
export async function generateQuestionsWithKIMI(
  params: GenerateQuestionsParams,
  onProgress?: (message: string) => void
): Promise<Question[]> {
  const apiKey = localStorage.getItem('kimi_api_key')
  const apiBase = localStorage.getItem('kimi_api_base') || 'https://api.siliconflow.cn/v1'
  const model = localStorage.getItem('kimi_api_model') || 'Pro/moonshotai/Kimi-K2.5'
