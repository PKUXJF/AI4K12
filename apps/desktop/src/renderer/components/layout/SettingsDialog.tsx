...
interface SettingsDialogProps {
   ...
   */
  initialTab?: 'providers' | 'connectors' | 'voice' | 'skills' | 'teacher' | 'appearance' | 'about';
}
...
export default function SettingsDialog({
  ...
  const [activeTab, setActiveTab] = useState<'providers' | 'connectors' | 'voice' | 'skills' | 'teacher' | 'appearance' | 'about'>(initialTab);
  const [appVersion, setAppVersion] = useState<string>('');
  const [skillsRefreshTrigger, setSkillsRefreshTrigger] = useState(0);
  const [devMode, setDevMode] = useState(() => {
    try { return localStorage.getItem('ai4edu_dev_mode') === 'true'; } catch { return false; }
  });
        ...
        <div className="space-y-6 mt-4">
          ...
          <div className="flex items-end justify-between border-b border-border">
            <div className="flex gap-4 flex-wrap">
              ...
              {/* === Developer-only tabs (hidden unless devMode) === */}
              {devMode && (
                <>
                  <div className="w-px h-6 bg-border self-center opacity-50" />
                  <button
              ...
              <div className="rounded-lg border border-border bg-card p-6">
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="font-medium">{appVersion || '加载中...'}</div>
                  </div>
                </div>
                <div className="mt-6 pt-4 border-t border-border">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-sm font-medium text-foreground">开发者模式</div>
                      <div className="text-xs text-muted-foreground">
                        显示高级选项（Providers、Skills、Connectors 等）
                      </div>
                    </div>
                    <button
                      onClick={() => {
                        const next = !devMode;
                        setDevMode(next);
                        localStorage.setItem('ai4edu_dev_mode', String(next));
                        // If turning off dev mode while on a dev tab, switch to teacher tab
                        if (!next && ['providers', 'connectors', 'skills', 'voice'].includes(activeTab)) {
                          setActiveTab('teacher');
                        }
                      }}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 ${devMode ? 'bg-primary' : 'bg-muted'}`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform duration-200 ${devMode ? 'translate-x-6' : 'translate-x-1'}`}
                      />
                    </button>
                  </div>
                </div>
